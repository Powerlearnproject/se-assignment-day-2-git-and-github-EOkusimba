...
Repositories are storage locations for project history and versions of a file.
Pull requests involve proposing and reviewing changes to the code.
Commits are snapshots of changes to the code, with unique IDs.
Branches are independent development lines that allows the modification of the code separately from the main code.
Merges involve combining changes to the code.
Conflicts are manual resolution needed when changes overlap.
Forks are copies of the code made for experimentation.
Clones are local copies of repositories
...
One needs to Sign up or sign in to GitHub. 
...
Use git clone in your terminal to clone the repository to your local machine.
Make changes, stage them with git add, and commit with git commit.
...
Setup Instructions: Steps for installation and usage.
Contribution Guidelines: How others can participate or submit changes.
Dependencies and License: Required tools and usage permissions.
...
Set Up Git – Install and verify Git with (git –version)
Clone or Initialize a Repository – Clone an existing repo (git clone <repo-url>) or start a new one (git init).
Add Files – Stage changes using (git add <file-name>) or (git add .)
Commit Changes – Save a snapshot with (git commit -m "Initial commit: Added project files")
Connect to GitHub – Link a remote repo with (git remote add origin <repo-url>)
Push to GitHub – Upload the commit using (git push -u origin main)
...
Create a Branch: Use (git checkout -b branch-name) to start a new feature or task.
Work on the Branch: Make changes, stage them (git add .), and commit (git commit -m "message").
Push Branch to GitHub: Share with the team via (git push origin branch-name)
Create a Pull Request: Propose merging the branch into the main codebase and allow for reviews.
Merge Branch: Once approved, merge into the main branch on GitHub or locally (git merge branch-name).
...
Encouraging Team Collaboration – Developers can provide feedback before merging.
Reducing Bugs & Errors – PRs ensure code is reviewed and tested before integration.
Enhancing Transparency – Maintains a history of discussions and changes.
Triggering automated tests to ensure stability.
Typical Steps in Creating & Merging a Pull Request
Create a Branch & Make Changes – Developers create a new branch, commit updates, and push it to GitHub.
Open a Pull Request – A PR is created on GitHub with a description of the changes.
Code Review & Discussion – Team members review, suggest modifications, and request updates if needed.
Approve & Merge – Once approved, the PR is merged into the main branch, and the feature branch can be deleted.
Sync Local Repository – Developers update their local main branch after merging.
...
Contributing to Open Source: Modify a project and submit changes via pull requests.
Experimentation: Safely test features or ideas without affecting the original.
Customization: Tailor an existing project to suit your specific needs.
Collaboration: Use forks to share a modified version with your team.
Learning: Study or practice coding with an isolated copy of the repository.
